...
export const Header = () => {
  ...
  const handleBuyNowClick = () => {
    if (isHomePage) {
      ...
      navigate('/');
      // We need a way to know when the home page is loaded to scroll
      // This is a simplified approach. A more robust solution might involve
      // a state or context to indicate the need to scroll after navigation.
      setTimeout(() => {
        scrollToPricing();
  ...
  const handleNavLinkClick = (to: string, sectionId?: string) => {
    if (to.startsWith('/#')) {
      navigate('/');
      setTimeout(() => {
        scrollToSection(sectionId as string);
      }, 100); // Adjust delay if needed
    } else {
      navigate(to);
    }
  ...
  const renderNavLink = (to: string, sectionId: string, text: string) => {
    return (
      <button onClick={() => handleNavLinkClick(to, sectionId)} className="text-gray-700 hover:text-blue-600 transition-colors">
        {text}
      </button>
    );
  };
  const renderMobileNavLink = (to: string, sectionId: string, text: string) => {
    return renderNavLink(to, sectionId, text); // Reuse desktop render logic
  };
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            ...
            {renderNavLink("/#features", "features", "Features")}
            {renderNavLink("/#services", "services", "Services")}
            {renderNavLink("/#how-it-works", "how-it-works", "How it Works")}
          ...
          <div className="hidden md:flex items-center space-x-4">
            <Button onClick={handleBuyNowClick} className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700">
              Buy Now
            </Button>
          </div>
        ...
        {isMenuOpen && (
          <div className="md:hidden py-4 border-t">
            <div className="flex flex-col space-y-4">
              ...
              {renderMobileNavLink("/#features", "features", "Features")}
              {renderMobileNavLink("/#services", "services", "Services")}
              {renderMobileNavLink("/#how-it-works", "how-it-works", "How it Works")}
              <Link to="/blog" className="text-left text-gray-700 hover:text-blue-600 transition-colors">
                Blog
              </Link>
              {renderMobileNavLink("/#contact", "contact", "Contact")}
              <Button onClick={handleBuyNowClick} className="w-full bg-gradient-to-r from-blue-600 to-purple-600">
                Buy Now
              </Button>
            </div>
          </div>
